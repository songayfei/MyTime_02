...
public class DateUtils {
    ...
    public static String getTimeStringDisplay(String getDateString) {
        final Context context = MyApplication.getMyinstance();
        Date getDate = null;
        try {
            getDate = getFormat(DATE_FORMAT_6).parse(getDateString);
        } catch (ParseException e) {
        ...
        if(getDate.getDate() < formatSysDate.getDate()){
            ...
            if (year > 0 || month > 0 || day > 0) {
                ...
                SimpleDateFormat simpleDateFormat = new SimpleDateFormat(
                        DATE_FORMAT_7);
                return simpleDateFormat.format(getDate);
//                 return 1 + context.getString(R.string.str_hoursago);
            } else  {
//               return "1" + context.getString(R.string.str_secondago);
                return context.getString(R.string.str_minsago_str); //都换成分钟前
            }
        }
...
//          return "1" + context.getString(R.string.str_secondago);
    ...
    public static boolean compareToDate(String oldDate, String defDate) {
        if(LogUtils.DEBUG){
            LogUtils.w("oldDate = " + oldDate + ", defDate" + defDate);
        }
...
//        }
     ...
     * @return
     */
    public static String formatNewThingDetailTime(String dateString) {
        return formatNewThingDetailTime(dateString, 0);
    }
    /**
     * @param dateString
     ...
     * @return
     */
    public static String formatNewThingDetailTime(String dateString, int type) {
        final Context context = MyApplication.getMyinstance();
        Date getDate = null;
        try {
            getDate = getFormat(DATE_FORMAT_2).parse(dateString);
        } catch (ParseException e) {
            e.printStackTrace();
            getDate = new Date();
        }
        ...
        if (LogUtils.DEBUG) {
            LogUtils.d(dateString + "= year " + year + ", month "
                    + month + ", day " + day + ", hours " + hours
                    + ", minute " + minute + ", seconds " + seconds
                    + ", 当前年：" + formatSysDate.getYear() + ", 发布年：" + getDate.getYear());
            LogUtils.i(dateString + "= 当前： 年=" + formatSysDate.getYear() + ", 月="
                    + formatSysDate.getMonth() + ", 日=" + formatSysDate.getDay());
            LogUtils.v(dateString + "= 传入： 年=" + getDate.getYear() + ", 月="
                    + getDate.getMonth() + ", 日=" + getDate.getDay());
        }
     ...
     * @return
     */
    public static String formatTweetDetailTime(String dateString, boolean isKtv) {
